...
cmd_chat: db 'key=$(tr -d "\r\n" < $HOME/.xai-key 2>/dev/null); [ -n "$key" ] || exit 10; curl -sS https://api.x.ai/v1/chat/completions -H "Authorization: Bearer $key" -H "Content-Type: application/json" -d @/tmp/qp_req.json > /tmp/qp_resp.json',0
